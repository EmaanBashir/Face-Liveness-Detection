...
class TextureDetect(object):
    def __init__(self, fold):
        ...
        self.texture_preds = deque(maxlen=60)
        self.endloop = False
        self.all_frames = []
        self.recorded = False
    ...
    def get_frame(self):
        ...
        if self.found_face and self.loops >= 25:
            ...
            if self.face.shape[0] > 0 and self.face.shape[1]>0:
                ...
                texture_pred = texture_utils.predict(self.face)
                self.texture_preds.append(texture_pred)
                
                self.mean_colors += [self.face.mean(axis=0).mean(axis=0)]
                self.timestamps  +=  [ret] if self.use_prerecorded else [time.time()]
                self.all_frames.append([f, texture_pred[0]])
        ...
        if self.mean_colors_resampled.shape[1] > self.window:
            ...
            bf_utils.putonframe(output, avg_bpm)
            avg_texture_pred = statistics.mean(self.texture_preds[0])
            texture_utils.put_result_onframe(output, 'Real' if avg_texture_pred <= 0.5 else 'Fake')
            self.endloop = True
            if not self.recorded:
                #video2_1_0_1 means bloodflow predicted 1 and texture predicted 0 and it was actually 1 for video 2
                self.bf_result = 0 if avg_bpm > 58 else 1
                self.texture_result = 0 if avg_texture_pred <= 0.5 else 1
                self.name += "_" + str(self.bf_result) + "_" + str(self.texture_result)
                self.recorded = True
