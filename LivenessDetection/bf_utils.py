...
class RecordingReader():
    def __init__(self,path = "static/recording_rest"):
        self.index = 0
...
def put_snr_bpm_onframe(bpm,snr,frame, avg_bpm):
    ...
    cv2.putText(frame,"Average BPM: " + str(avg_bpm),(0, 100),font,fontScale,color,thickness)

    cv2.putText(frame, "Blood Flow Analysis: " + ("Real" if avg_bpm > 58 else "Fake"), (0, 200),font,fontScale,color,thickness)

def putonframe(frame, avg_bpm):
    text = "Blood Flow Analysis: " + ("Real" if avg_bpm > 58 else "Fake")
    font = cv2.FONT_HERSHEY_SIMPLEX 
    org = (00, 50) 
    fontScale = 1
    color = (0, 0, 255) 
